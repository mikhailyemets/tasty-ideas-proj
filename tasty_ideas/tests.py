...
from tasty_ideas.forms import MainPageSearchForm, DishSearchForm, CookCreateForm, CookForm, DishForm
...
class DishListViewTest(TestCase):
    ...
    def test_dish_list_view_with_search_query(self):
        ...
        response = self.client.get(reverse('tasty_ideas:dish-list', kwargs={'pk': self.category.pk}), {'query': search_query})
...
class DishDetailViewTest(TestCase):
    ...
    def test_dish_detail_view(self):
        response = self.client.get(reverse('tasty_ideas:dish-detail', kwargs={'pk': self.dish.pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_dish_detail_view_with_invalid_dish_id(self):
        ...
        response = self.client.get(reverse('tasty_ideas:dish-detail', kwargs={'pk': invalid_dish_id}))
    ...
    def test_dish_detail_view_reviews(self):
        Review.objects.create(dish=self.dish,
                              left_by=get_user_model().objects.create(
                                  username='user1'), content='Review 1')
        Review.objects.create(dish=self.dish,
                              left_by=get_user_model().objects.create(
                                  username='user2'), content='Review 2')
...
class DishCreateViewTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.category = Category.objects.create(dish_type='Test Dish Type')
        cls.user = get_user_model().objects.create_user(username='test_user', password='testpassword')

    def test_dish_create_view(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('tasty_ideas:dish-create', kwargs={'pk': self.category.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTrue('form' in response.context)
...
class DishUpdateViewTest(TestCase):
    ...
    def test_dish_update_view(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('tasty_ideas:dish-update', kwargs={'pk': self.dish.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTrue('form' in response.context)
    ...
    def test_dish_update_view_redirect_when_not_authenticated(self):
        ...
        response = self.client.post(reverse(
            'tasty_ideas:dish-update', kwargs={'pk': self.dish.pk}
        ),data)
...
class DishDeleteViewTest(TestCase):
    ...
    def test_dish_delete_view(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('tasty_ideas:dish-delete', kwargs={'pk': self.dish.pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_dish_delete_view_post(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse('tasty_ideas:dish-delete', kwargs={'pk': self.dish.pk}))
        self.assertEqual(response.status_code, 302)
...
class DeleteReviewViewTest(TestCase):
    def setUp(self):
        ...
        self.dish = Dish.objects.create(name='Test Dish', price=10.99, spicy='easy',cooking_time=5, category=self.category, difficulty='easy', recipe='Test recipe')
        self.review = Review.objects.create(dish=self.dish, left_by=self.user, content='Test Review')

    def test_delete_review_by_anonymous_user(self):
        self.client.logout()
        response = self.client.post(reverse('tasty_ideas:delete_review', kwargs={'pk': self.review.pk}))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(Review.objects.count(), 1)
    ...
    def test_delete_review_with_invalid_review_id(self):
        ...
        response = self.client.post(reverse('tasty_ideas:delete_review', kwargs={'pk': invalid_review_id}))
        self.assertEqual(response.status_code, 404)
        self.assertEqual(Review.objects.count(), 1)
...
class UserProfileViewTest(TestCase):
    ...
    def test_user_profile_form_submission(self):
        ...
        response = self.client.post(reverse('tasty_ideas:user_profile'), {
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(get_user_model().objects.get(id=self.user.id).username, new_username)
        self.assertEqual(get_user_model().objects.get(id=self.user.id).first_name, 'New')
...
class CookModelTest(TestCase):
    def test_cook_creation(self):
        cook = get_user_model().objects.create_user(username='testcook', first_name='Test', last_name='Cook', password='testpass123', experience=5)
...
class DishFormTest(TestCase):
    ...
    def test_dish_form_valid_data(self):
        ...
        self.assertEqual(dish.difficulty, 'easy')
        self.assertEqual(dish.recipe, 'Test recipe')
        self.assertQuerysetEqual(dish.ingredients.all(), [self.ingredient1, self.ingredient2], transform=lambda x: x)
